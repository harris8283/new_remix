...
contract ERC20token {
    string public constant name = "RRT token";        //回傳token的名稱為Rick Roll
    ...
    address payable public owner;           //合約建構者
    ...
    event Received (address _sender, uint _value, string _message);         //Received事件負責接收ETH
    ...
    constructor() {
        owner=payable(msg.sender);       //設定合約建構者
    }
    ...
    receive () external payable{          //若msg.data不為空則執行receive
        paytoken();         //發送token
        emit Received(msg.sender, msg.value, "receive was called");         //接收ETH
    }

    function paytoken() private returns(bool){
        if(totalSupply > 0                      //確定發行額足夠
        && balances[msg.sender] ==0             //確定帳戶內為空
        && msg.value == 0.001 ether             //判定接收數量
        && complete[msg.sender] < 1000){        //判定位址是否參與ico過
            totalSupply -= 1000;                //將尚可發行額減本次發行數量
            balances[msg.sender] = 1000;        //紀錄帳號內已經有1000個token
            complete[msg.sender] = 1000;        //紀錄帳號已經完成ico過了
            emit Transfer(address(this), msg.sender, 1000);         //實際發送token
            return true;
        }else {
            revert();       ////放棄執行，還原狀態
        }
