...
contract customer is Owner{
    ...
    constructor(){
        ...
        staking_account = payable(msg.sender);
    }
    ...
    function change_staking_account(address payable _address) onlyOwner public{
        staking_account = _address;
    }

    function add_customer(address _customer_address) payable public{       //新增客戶資訊 並將ETH轉入專用帳戶 建議可用hash替換位址(由客戶支出ETH的address使用)(需改善問題點為若是客戶自行私下與合約交互將會使公司記錄與智能合約不符)
        ...
        require(msg.value > 0,"Didn't supply ETH");
        ...
        staking_account.transfer(msg.value);
        amount += 1;
    } 

    function delete_customer(address payable _customer_address) onlyManager payable public{       //刪除客戶資訊同時歸還質押金額(由大哥大address使用)
        require(customer_map[_customer_address].customer_address != address(0),"This account isn't exist");
        require(msg.value >= customer_map[_customer_address].value + customer_map[_customer_address].add_value,"We need pay off the staking to the customer");
        if (customer_map[_customer_address].value + customer_map[_customer_address].add_value != 0){
            _customer_address.transfer(customer_map[_customer_address].value + customer_map[_customer_address].add_value);
        }
        delete account[customer_map[_customer_address].id];
        delete customer_map[_customer_address];
    }

    function change_staking_value(address _customer_address, uint _value) onlyOwner public{             //(由大哥大address使用)
        require(customer_map[_customer_address].customer_address != address(0),"This account isn't exist");
        customer_map[_customer_address].value = _value;          //更改原本的質押金額
    }  

    function change_staking_add_value(address _customer_address, uint _value) onlyOwner public{         //(由大哥大address使用)
        require(customer_map[_customer_address].customer_address != address(0),"This account isn't exist");
        customer_map[_customer_address].add_value = _value;          //更改新增的質押金額
    }  

    function increase_staking_value(address _customer_address) payable public{           //增加質押金額並轉帳(由客戶address使用)
        require(customer_map[_customer_address].customer_address != address(0),"This account isn't exist");
        require(msg.value > 0,"Didn't supply ETH");
        customer_map[_customer_address].add_value += msg.value;                                         //置換為新的質押金額
        customer_map[_customer_address].newtime = block.timestamp;
        staking_account.transfer(msg.value);
    }

    function reduce_staking_value(address _customer_address) onlyManager payable public{         //減少質押金額並轉帳(由大哥大address使用)
        require(customer_map[_customer_address].customer_address != address(0),"This account isn't exist");             //確定是否位址已存在
        require(msg.value <= customer_map[_customer_address].value + customer_map[_customer_address].add_value, "Staking value is not enough");       //確認提取金額是否超支
        require(msg.value > 0,"Didn't supply ETH");
        if (msg.value > customer_map[_customer_address].add_value){
            customer_map[_customer_address].add_value = 0;
            uint _value = msg.value - customer_map[_customer_address].add_value;
            customer_map[_customer_address].value -= _value;
        }else{
            customer_map[_customer_address].add_value -= msg.value;
        }
